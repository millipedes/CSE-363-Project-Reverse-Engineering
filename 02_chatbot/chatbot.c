...
int print_words(){
	if(h==1){
		...
		//segfaults here but not always
		//printf("\t'%s'\n", word_search[0]); 
  		search_faculty(word_search[0]);
  		//printf("Ding\n");
		//puts(htable_get( htable, word_search[0]));
		fputs(htable_get( htable, word_search[0]), stdout);
		fputc(10, stdout); //added 2 lines, removed one
		}
		// h=0;
		// printf(">>>");
		// gets(c);
     ...
     printf(">>>");
		 ...
		 if(l==ch){
			 ...
			 if(ch==1){
				...
				fputs(htable_get( htable, word_search2[gh]), stdout);
				fputc(10, stdout); //added 2 lines, removed one
	 		}
		 gets(c);
		 // h=0;
